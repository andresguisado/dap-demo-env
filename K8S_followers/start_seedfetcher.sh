...
update_conjur_config_map() {
  ...
  $CLI -n $CONJUR_NAMESPACE_NAME delete cm ${CONJUR_CONFIG_MAP} --ignore-not-found

  # Append Follower cert to config map 
  cat << EOL3 | $CLI -n $CONJUR_NAMESPACE_NAME apply -f -
apiVersion: v1
kind: ConfigMap
metadata:
  name: "${CONJUR_CONFIG_MAP}"
